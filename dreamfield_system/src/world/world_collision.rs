...
use crate::{world::{world_chunk::{ChunkIndex, VERTEX_STRIDE, INDEX_STRIDE, WorldChunk}, WorldChunkManager, aabb::Aabb}, math::intersection::Capsule};
use cgmath::{Vector3, vec3, ElementWise};

use crate::math::intersection::{self, Triangle};
...
impl WorldCollision {
    ...
    /// Sweep a capsule out from start to end, returning an intersection result along with a time of impact
    pub fn sweep_capsule(&mut self, world: &mut WorldChunkManager, capsule: &Capsule, velocity: Vector3<f32>,
        radius: f32) -> Option<SpherecastResult>
    {
        // Apply change of basis from R3 to ellipsoid space
        let cbm = vec3(1.0 / radius, 1.0 / radius, 1.0 / radius);

        let a = capsule.a.mul_element_wise(cbm);
        let b = capsule.b.mul_element_wise(cbm);
        let velocity = velocity.mul_element_wise(cbm);

        // Perform intersection
        let mut result = self.sweep_unit_capsule(world, a, b, velocity, cbm);
    ...
    /// Sweep a capsule with a radius of 1 and a given change of basis matrix
    pub fn sweep_unit_capsule(&mut self, world: &mut WorldChunkManager, a: Vector3<f32>, b: Vector3<f32>,
        velocity: Vector3<f32>, cbm: Vector3<f32>) -> Option<SpherecastResult>
    {
        // Construct an aabb for the sphere's path
        let mut sphere_path_aabb = Aabb::new();
        sphere_path_aabb.expand_with_point(&(a - vec3(1.0, 1.0, 1.0)));
        sphere_path_aabb.expand_with_point(&(a + vec3(1.0, 1.0, 1.0)));
        sphere_path_aabb.expand_with_point(&(b - vec3(1.0, 1.0, 1.0)));
        sphere_path_aabb.expand_with_point(&(b + vec3(1.0, 1.0, 1.0)));
        sphere_path_aabb.expand_with_point(&(a + velocity - vec3(1.0, 1.0, 1.0)));
        sphere_path_aabb.expand_with_point(&(a + velocity + vec3(1.0, 1.0, 1.0)));
        sphere_path_aabb.expand_with_point(&(b + velocity - vec3(1.0, 1.0, 1.0)));
        sphere_path_aabb.expand_with_point(&(b + velocity + vec3(1.0, 1.0, 1.0)));
        ...
        for x in chunk_min_x..=chunk_max_x {
            for z in chunk_min_z..=chunk_max_z {
                if let Some((_, meshes)) = self.get_chunk_meshes(world, (x, z)) {
                    // Check each mesh in the chunk for intersections
                    for (_, mesh) in meshes.iter() {
                        for triangle in mesh.iter() {
                            let triangle = triangle.apply_cbm(cbm);

                            let res = intersection::toi_unit_capsule_triangle(a, b, velocity, &triangle);
                            if let Some((toi, _, _)) = res {
                                if let Some((closest_toi, _, _)) = closest_intersection {
                                    if toi >= 0.0 && toi < closest_toi {
                                        closest_intersection = res;
