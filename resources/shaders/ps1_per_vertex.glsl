...
void main() {
    ...
    var_dist = length(gl_Position.xyz);

    // Snap to 320x240 grid
    // TODO: add uniform for this
    const vec2 grid = vec2(320, 240);
    vec2 snapped = gl_Position.xy / gl_Position.w;
    snapped.xy = floor(grid * snapped.xy) / grid;
    gl_Position.xy = snapped * gl_Position.w;

    var_light = calc_lighting();
}
...
void main() {
    ...
    vec3 out_color = var_light * base_color;
